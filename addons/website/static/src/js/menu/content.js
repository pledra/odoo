...
var PagePropertiesDialog = widget.Dialog.extend({
    ...
    init: function (parent, page_id, options) {
        ...
        var length_url = server_url.length;
        var server_url_trunc = server_url;
        if(length_url > 30)
            server_url_trunc = server_url.slice(0,14) + '..' + server_url.slice(length_url-14,length_url);
        self.server_url = server_url; 
        self.server_url_trunc = server_url_trunc;
        self.current_page_url = window.location.pathname;
        self.page_id = page_id;
        self._super(parent, _.extend({}, {
...
var MenuEntryDialog = widget.LinkDialog.extend({
    ...
    _hideOptions: function(_this,to_show) {
        ...
        if (to_show == 'url') {
            ...
            _this.$('#o_link_dialog_existing_page_input').parents('.form-group').show();
        }
    },
});
