...
class WebsiteBlog(http.Controller):
    ...
    def nav_list(self, blog=None):
        ...
        unsorted = {year: [m for m in months] for year, months in itertools.groupby(groups, lambda g: g['year'])}
        return OrderedDict(sorted(unsorted.items(), key=lambda t: int(t[0]), reverse=True))
