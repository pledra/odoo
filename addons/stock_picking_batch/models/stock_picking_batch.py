...
class StockPickingBatch(models.Model):
    ...
    def done(self):
        ...
        if any(picking.state not in ('assigned', 'partially_available') for picking in pickings):
        ...
        for picking in pickings:
            if picking.state == 'draft' or all([x.qty_done == 0.0 for x in picking.move_line_ids]):
