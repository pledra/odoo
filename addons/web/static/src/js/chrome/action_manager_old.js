odoo.define('web.ActionManagerOld', function (require) {
"use strict";
...
var ActionManager = Widget.extend({
    ...
    push_action: function(widget, action_descr, options) {
        ...
        return $.when(this.inner_widget.appendTo(new_widget_fragment)).done(function() {
            ...
            self.toggle_fullscreen(); // AAB: to reimplement
    ...
    do_action: function(action, options) {
        ...
        core.bus.trigger('action', action); // AAB: is this useful? -> no
        ...
        // AAB: the following if statement done in the controller?
        ...
        // AAB: this is some crap we need to get rid off (move this to board addon)
        if (action.res_model === 'board.board' && action.view_mode === 'form') {
            ...
            // AAB: this will be done in the controller?
    ...
    ir_actions_client: function (action, options) {
        ...
        return this.ir_actions_common({
            ...
            action: action,
            klass: 'oe_act_client', // AAB: unused here
    ...
    ir_actions_act_window_close: function (action, options) {
        if (!this.dialog) {
            options.on_close(); // AAB: is it necessary?
        }
        this.dialog_stop();
    ...
    ir_actions_report: function(action, options) {
        // AAB: is this dead code (due to the override in qwebactionmanager)? there is no other
        // report_type than qweb-html and qweb-pdf (so backward compat? can be dropped in v12?)
        var self = this;
        framework.blockUI();
