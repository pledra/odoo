...
from odoo.addons.mail.tests.common import BaseFunctionalTest, TestMail, MockEmails
...
class TestMailgateway(BaseFunctionalTest, MockEmails):
    ...
    def test_message_process_partner_find(self):
        ...
        from_2 = self.env['res.users'].with_context({'no_reset_password': True}).create({'name': 'Brice Denisse', 'login': 'B', 'email': 'from.test@example.com'})
