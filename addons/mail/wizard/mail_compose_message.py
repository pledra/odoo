...
class MailComposer(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        result['template_id'] = self.env['ir.values'].sudo().get_default('mail.compose.message', 'template_id', condition='mail_compose_act_btn=' + (self.env.context.get('mail_compose_act_btn') or '') + '&' + 'model=' + (result['model'] or '')) or result.get('template_id')
    ...
    def _compute_is_default_template(self):
        ...
        for compose in self:
            template_id = IrValues.get_default('mail.compose.message', 'template_id', condition='mail_compose_act_btn=' + (self.env.context.get('mail_compose_act_btn') or '') + '&' + 'model=' + (compose.model or ''))
    ...
    def mark_template_as_default(self):
        ...
        if self.template_id and self.env.context.get('mail_compose_act_btn'):
            self.env['ir.values'].sudo().set_default('mail.compose.message', "template_id", self.template_id.id, condition="mail_compose_act_btn=" + (self.env.context.get('mail_compose_act_btn')) + '&' + 'model=' + (self.model or ''))
