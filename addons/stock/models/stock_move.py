        ...
        for move in self.filtered(lambda m: m.state in ['confirmed', 'waiting', 'partially_available']):
            if move.location_id.usage in ('supplier', 'inventory', 'production', 'customer')\
                ...
                if not move.move_orig_ids:
                    ...
                    for k, g in groupby(sorted(move_lines_in, key=_keys_in), key=itemgetter(*keys_in)):
                        qty_done = 0
                        for ml in g:
                    ...
                    for k, g in groupby(sorted(move_lines_out_done, key=_keys_out), key=itemgetter(*keys_out)):
                        qty_done = 0
                        for ml in g:
                            qty_done += ml.product_uom_id._compute_quantity(ml.qty_done, ml.product_id.uom_id)
                        grouped_move_lines_out[k] = qty_done
                    for k, g in groupby(sorted(move_lines_out_reserved, key=_keys_out), key=itemgetter(*keys_out)):
