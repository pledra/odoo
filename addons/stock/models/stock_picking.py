...
class Picking(models.Model):
    ...
    def check_backorder(self):
        ...
        for ops in self.mapped('move_lines').mapped('move_line_ids').filtered(lambda x: x.package_id and not x.product_id and not x.move_id):
            for quant in ops.package_id.quant_ids:
                quantity_done.setdefault(quant.product_id.id, 0)
                quantity_done[quant.product_id.id] += quant.qty
        for pack in self.mapped('move_lines').mapped('move_line_ids').filtered(lambda x: x.product_id and not x.move_id):
