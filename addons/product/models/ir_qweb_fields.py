...
class WeightConverter(models.AbstractModel):
    ...
    def record_to_html(self, record, field_name, options):
        ...
        weight_uom = self.env.user.company_id.weight_uom_id
        ...
        if 0 < weight_uom.rounding < 1:
            rounding = int(math.ceil(math.log10(1.0 / weight_uom.rounding)))
        else:
            rounding = 0

        options.update({'name': weight_uom.name, 'rounding': rounding})
...
class VolumeConverter(models.AbstractModel):
    ...
    def record_to_html(self, record, field_name, options):
        ...
        volume_uom = self.env.user.company_id.volume_uom_id
        ...
        if 0 < volume_uom.rounding < 1:
            rounding = int(math.ceil(math.log10(1.0 / volume_uom.rounding)))
        else:
            rounding = 0

        options.update({'name': volume_uom.name, 'rounding': rounding})
