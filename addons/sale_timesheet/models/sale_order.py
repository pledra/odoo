...
class SaleOrderLine(models.Model):
    ...
    def _compute_qty_delivered_method(self):
        ...
        # prefetch field before filtering, to avoid fetching one by one product type
        self.mapped('product_id')
        # set status for my concerned records only.
        lines_by_timesheet = self.filtered(lambda sol: sol.product_id.type == 'service' and sol.product_id.service_type == 'timesheet')
        for line in lines_by_timesheet:
            line.qty_delivered_method = 'timesheet'
        # call super with remaining records (delegate the job to super)
        # prefetch optimization comming from https://github.com/odoo/odoo/commit/59b010ac366e0826fb314b2a89ca47f26906e185
        super(SaleOrderLine, (self - lines_by_timesheet).with_prefetch(self._prefetch))._compute_qty_delivered_method()

    @api.multi
    @api.depends('analytic_line_ids.project_id', 'analytic_line_ids.unit_amount')
    def _compute_qty_delivered_auto(self):
        ...
        for line in lines_by_timesheet:
            line.qty_delivered_auto = mapping.get(line.id, 0.0)
