...
var is_ready = openerp.website.ready();
$('.oe_website_sale').each(function () {
    ...
    $(oe_website_sale).find(".oe_cart input.js_quantity").on("change", function () {
        ...
        .then(function (res) {
            //basic case
            price_to_str(res[product_id]).then(function (price) { $dom.find('span.oe_currency_value').last().text(price); });
            ...
            $dom_optional.each(function(){
                ...
                price_to_str(res[id]).then(function(price) { $(this).find("span.oe_currency_value").last().text(price); });
    ...
    $('form.js_add_cart_json label', oe_website_sale).on('mouseup touchend', function (ev) {
        ...
        price_to_str(value).then(function(price) { $price.html(price); });
    });
    ...
    function insert_thousand_seps(num) {
        ...
        return (negative ? '-' : '') + intersperse(
            num, JSON.parse(l10n.grouping), l10n.thousands_sep);
    }
    ...
    function price_to_str(price) {
        ...
        return $.when(is_ready.done()).then(function() {
    ...
    $(oe_website_sale).on('change', 'input.js_product_change', function (ev) {
        ...
        price_to_str(+$(this).data('lst_price')).then(function(price) { $parent.find(".oe_default_price:first .oe_currency_value").html(price); });
        price_to_str(+$(this).data('price')).then(function(price) { $parent.find(".oe_price:first .oe_currency_value").html(price); });
    ...
    $(oe_website_sale).on('change', 'input.js_variant_change, select.js_variant_change, ul[data-attribute_value_ids]', function (ev) {
        ...
        for (var k in variant_ids) {
            if (_.isEmpty(_.difference(variant_ids[k][1], values))) {
                price_to_str(variant_ids[k][2]).then(function (price) { $price.html(price); });
                price_to_str(variant_ids[k][3]).then(function (price) { $default_price.html(price); });
