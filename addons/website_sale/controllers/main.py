...
class WebsiteSale(http.Controller):
    ...
    def checkout_form_validate(self, mode, all_form_values, data):
        ...
        required_fields += getattr(self, '_get_mandatory_%s_fields' % mode[1], lambda x: [])() # from controller
        # Chekc if state required
        if data.get('country_id') and data.get('country_id') != '':
            country = request.env['res.country'].browse(int(data.get('country_id')))
            country_data = self.change_country(country)
            if 'state_code' in country_data.get('fields') and len(country_data.get('states')):
                required_fields += ['state_id']

        # error message for empty required fields
        for field_name in required_fields:
            if not data.get(field_name):
                print '%s field_name missing' % field_name
    ...
    def _checkout_form_save(self, mode, checkout, all_values):
        ...
        if mode[0] == 'new':
            ...
            if partner_id:
                ...
                print "###"
                print "###"
                print "checkout_form_save: no partner id while editing"
                print "###"
                print "###"
                print '\033[0m'
        return partner_id
    ...
    def values_postprocess(self, order, mode, values, errors, error_msg):
        ...
        for k, v in values.items():
            if k in authorized_fields and v is not None and v != '':
                ...
                if k not in ('field_required', 'partner_id', 'callback', 'submit_address'): # classic case
                    print '\033[93m' + ("postprocess: %s value has been dropped (empty or not writable)" % k) + '\033[0m'
        ...
        if 'submit_address' in kw:
            ...
            if errors:
                ...
                if not partner_id:
                    print "###"
                    print "###"
                    print "checkout_form_save return no partner ID !"
                    print "###"
                    print "###"
