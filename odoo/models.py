    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    def write(self, vals):
        ...
        * .. _openerp/models/relationals/format:
        ...
        bad_names = set(['id', 'parent_left', 'parent_right'])
        if self._log_access:
    ...
    def _write(self, vals):
        ...
        updated = []                    # list of updated columns
        other_names = []                # list of non-column field names
        ...
        self.modified(updated + other_names)
        ...
        other_context = {key: val
                         for key, val in self._context.items()
                         if not key.startswith('default_')}

        for field in sorted(other_fields, key=attrgetter('_sequence')):
            field.write(self.with_context(other_context), vals[field.name])

        # mark fields to recompute
        self.modified(other_names)
    ...
    def _create(self, vals):
        ...
        with self.env.protecting(protected_fields, self):
            ...
            for field in sorted(other_fields, key=attrgetter('_sequence')):
                other_record = self.with_context(other_context)
                field.write(other_record, vals[field.name], create=True)
